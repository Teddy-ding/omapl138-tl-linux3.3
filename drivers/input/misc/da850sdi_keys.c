/* 
 * da850sdi_keys.c - support IR remote on DA850 SDI board
 *
 * Cloned from dm365evm_keys.c 
...
#define MSP430_GPIO		GPIO_TO_PIN(2, 2)
...
static irqreturn_t da850sdi_keys_irq(int irq, void *_keys)
{
	struct da850sdi_keys	*keys = _keys;
	...
	while (gpio_get_value(MSP430_GPIO) == 0) {
		...
		keycode = KEY_UNKNOWN;
		for (i = 0; i < ARRAY_SIZE(da850sdi_keys); i++) {
			if (da850sdi_keys[i].event != event)
				continue;
			keycode = da850sdi_keys[i].keycode;
			break;
		}
 ...
 */
static irqreturn_t da850sdi_keys_hardirq(int irq, void *_keys)
{
...
static int dm365evm_setkeycode(struct input_dev *dev, int index, int keycode)
{
	...
	if (((unsigned)index) >= ARRAY_SIZE(da850sdi_keys))
		return -EINVAL;

	old_keycode = da850sdi_keys[index].keycode;
	da850sdi_keys[index].keycode = keycode;
	set_bit(keycode, dev->keybit);

	for (i = 0; i < ARRAY_SIZE(da850sdi_keys); i++) {
		if (da850sdi_keys[index].keycode == old_keycode)
			goto done;
	}
...
static int dm365evm_getkeycode(struct input_dev *dev, int index, int *keycode)
{
	...
	return da850sdi_keys[index].keycode;
}

/*----------------------------------------------------------------------*/

static int __devinit
da850sdi_keys_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	struct da850sdi_keys	*keys;
	struct input_dev	*input;
	int			status;
	int			i;
	...
	input->name = "DA850 SDI Controls";
	input->phys = "da850sdi/input0";
	...
	for (i = 0; i < ARRAY_SIZE(da850sdi_keys); i++)
		__set_bit(da850sdi_keys[i].keycode, input->keybit);
	...
	status = request_threaded_irq(gpio_to_irq(MSP430_GPIO),
			da850sdi_keys_hardirq, da850sdi_keys_irq,
...
static int __devexit da850sdi_keys_remove(struct i2c_client *i2c)
{
	struct da850sdi_keys	*keys = i2c_get_clientdata(i2c);
...
static struct i2c_driver da850sdi_keys_driver = {
	.driver.name	= "da850sdi_keys",
	.id_table	= ids,
	.probe		= da850sdi_keys_probe,
	.remove		= __devexit_p(da850sdi_keys_remove),
};

static int __init da850sdi_keys_init(void)
{
	return i2c_add_driver(&da850sdi_keys_driver);
}
module_init(da850sdi_keys_init);

static void __exit da850sdi_keys_exit(void)
{
	i2c_del_driver(&da850sdi_keys_driver);
}
module_exit(da850sdi_keys_exit);
